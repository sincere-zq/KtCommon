...
import com.witaction.common.utils.DensityUtils.dp2px
import com.witaction.common.utils.DensityUtils.screenHeight
...
class AutoPlayScrollListener(
    ...
    private val rangeTop: Float,
    private val rangeBottom: Float
        ...
        recyclerView?.run {
            ...
            for (i in 0 until visibleCount) {
                if (layoutManager!!.getChildAt(i) != null && layoutManager.getChildAt(i)!!.findViewById<View?>(
                    ...
                    val player =
                        layoutManager.getChildAt(i)!!.findViewById<View>(itemPlayId) as GSYBaseVideoPlayer
                    val rect = Rect()
    ...
    private fun showWifiDialog(gsyBaseVideoPlayer: GSYBaseVideoPlayer, context: Context) {
        if (!NetworkUtils.isAvailable(context)) {
            Toast.makeText(context, context.resources.getString(R.string.no_net), Toast.LENGTH_LONG)
                .show()
            return
        }
    ...
    companion object {
         ...
         * 指定自动播放，在屏幕上的区域范围，上
         */
        val PLAY_RANGE_TOP = screenHeight / 2 - dp2px(200f)

        /**
         * 指定自动播放，在屏幕上的区域范围，下
         */
        val PLAY_RANGE_BOTTOM = screenHeight / 2 + dp2px(200f)
    }
